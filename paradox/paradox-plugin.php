...
class ParadoxPlugin {
  ...
  public function __construct() {
    ...
    // Activation Function, run on first time
    add_action('init', array(&$this, 'Activation'));
    
    // Run everytime the user saves with ACF
  ...
  public function addACF() {
    ...
    // Load any custom fields we might need
    $this->acfAddFields();
  ...
  /**
   * Add ACF extra fields types to be used in our admin pages
   */
  ...
  /**
   * Check if we need to run our activation function
   */
  public function Activation() {
    
    // Run default activation
    $ran = get_option($this->id.'activation');
    
    // If never run, set as runned.
    if (!$ran) {
      // Set as Runned
      update_option($this->id.'activation', true);
